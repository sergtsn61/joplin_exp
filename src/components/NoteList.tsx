...
export function NoteList() {
  ...
  useEffect(() => {
    if (!multiMode) return;
    const handler = (e: KeyboardEvent) => { if (e.key === 'Escape') exitMultiMode(); };
  ...
  return (
    <div className="flex flex-col h-full bg-gray-900 border-r border-gray-800">
      ...
      {multiMode && (
        <div className="flex items-center gap-2 px-3 py-1.5 bg-blue-950/50 border-b border-blue-900/50 shrink-0">
          <CheckCheck className="w-3.5 h-3.5 text-blue-400 shrink-0" />
          <span className="text-xs font-semibold text-blue-300 flex-1">
            {selected.size > 0 ? `${selected.size} selected` : 'Multi-select'}
          </span>
          <button
            onClick={() => setSelected(new Set(notes.map(n => n.id)))}
            className="text-xs text-blue-400 hover:text-blue-200 transition-colors px-1"
            title="Select all"
          >All</button>
          <button
          ...
          {selected.size > 0 && (
            <div className="relative">
              <button
                ...
                className="flex items-center gap-1 px-2 py-1 bg-blue-700 hover:bg-blue-600 rounded-lg text-xs transition-colors"
              >
